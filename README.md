# Grassr00tz 2026

Presentation and lab materials for the **Grassr00tz - 2026, Appleton WI**.

## About

This repository contains the speaker materials for a talk and hands-on lab focused on operational technology (OT) and industrial control system (ICS) cybersecurity. The session covers impact in OT environments, the role of failsafes and engineering controls, SCADA design considerations, and a practical Modbus exercise.

## Repository Structure

```
.
├── Presentation/           # Talk content
├── FrostyGoop/             # Live Example of how FrostyGoop worked, requires PLCs
├──── ignition_backup/      # Backup of Ignition project
├──── netshoot/             # Simulated compromised edge router
├──── PLC/                  # PLC code (Productivity 5000 & Click Plus)
└── README.md
```

## Audience

Students and industry experts attending Grassr00tz 2026.

## Status

Work in progress — content is being developed ahead of the 2026 talk.
